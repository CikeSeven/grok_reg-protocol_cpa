...
def _run_vm_via_node(chat_req: dict, xor_key: str, vm_type: str, flow: str = "oauth_create_account") -> Optional[str]:
    ...
    script_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sentinel_vm")
    ...
    # 写唯一临时文件 (避免并发冲突)
    fd, input_file = tempfile.mkstemp(suffix=".json", prefix="sentinel_", dir=script_dir)
    try:
...
class SentinelTokenProvider:
    ...
    async def get_token(self, flow: str, device_id: str) -> Optional[dict]:
        ...
        """
        # 检查缓存是否过期 (9分钟)
        if not self._cached_chat_req or time.time() - self._last_fetch_time > 540:
