...
3. **本地 WebUI 运维台**  
   `uv run python -m webui` 启动 `http://127.0.0.1:8787`：注册控制、账号账本、CPA 管理、Hotmail 凭证、微软邮箱管理、配置中心、任务日志。视觉风格参考 `gpt_oauth`，业务按本项目文件账本组织。
...
原运维台保持在 `/`；精简的 Soft UI / 漫画监控看板位于 `/dash`。
...
| 工具 | 账号格式转换，以及独立的微软邮箱检测、验证码和邮件浏览 |
...
- 工具页使用独立的 `mail_tool_credentials.txt`，不会读取、导入或改写注册使用的 `mail_credentials.txt`。
- 支持 TXT、CSV、JSON/JSONL 等常见凭证结构，自动识别密码、Client ID 和 refresh token。
- 自动检测 XOAUTH2 IMAP 或 Microsoft Graph，可读取最近验证码，也可按文件夹和页码查看邮件正文。
- 邮件正文只随当前 API 响应返回，不写入 `mail_tool_state.json`；该状态文件只保留协议和健康检查概览。
- refresh token 发生轮换时只回写独立邮箱库，并通过比较后更新避免覆盖刚导入的新凭证。
...
部署包只包含 Git 跟踪文件，不包含被 `.gitignore` 排除的账号、配置和运行状态。服务器脚本还会二次拒绝 `config.json`、`accounts_*.txt`、`mail_credentials.txt`、`mail_tool_credentials.txt`、`cpa_auths/` 等运行态文件。每次部署会备份即将覆盖的文件，执行 `uv sync --frozen`，重启服务并轮询健康检查；失败时自动恢复备份并重启旧版本。
